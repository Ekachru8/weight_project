...
import { generateExerciseImage } from "../src/lib/image-generator";
import fs from "fs";
...
async function main() {
  ...
  for (const exercise of EXERCISES) {
    ...
    if (existing?.imageStatus === "ready" && existing?.imageUrl) {
      console.log(`[SKIPPED] ${exercise.slug} already has an image.`);
      skippedCount++;
      continue;
    }
    ...
    if (!success) {
      console.error(`[FAILED] Could not generate image for ${exercise.slug} after 3 attempts.`);
      failedCount++;
      updatedAssets[exercise.slug] = {
        ...existing,
        imageStatus: "failed"
